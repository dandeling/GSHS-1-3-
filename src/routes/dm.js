...
router.get('/:username', requireAuth, async (req, res) => {
  const other = await db.prepare('SELECT id, username, point, role FROM users WHERE username=?').get(req.params.username);
  ...
  await db.prepare('UPDATE dm_messages SET is_read=1 WHERE sender_id=? AND recipient_id=? AND is_read=0').run(other.id, me);
  res.json({ other, messages: msgs, meId: me });
});

// 쪽지 보내기
router.post('/:username', requireAuth, async (req, res) => {
  const other = await db.prepare('SELECT id, username FROM users WHERE username=?').get(req.params.username);
  if (!other) return res.status(404).json({ error: '사용자를 찾을 수 없습니다.' });
  if (other.id === req.user.id) return res.status(400).json({ error: '자기 자신에게는 보낼 수 없어요.' });
  const { content } = req.body || {};
  if (!content?.trim()) return res.status(400).json({ error: '내용을 입력하세요.' });
  const info = await db.prepare('INSERT INTO dm_messages (sender_id, recipient_id, content) VALUES (?, ?, ?)')
    .run(req.user.id, other.id, content.trim());
  await notify(other.id, req.user.id, 'dm', null, `쪽지: "${content.slice(0, 40)}"`);
