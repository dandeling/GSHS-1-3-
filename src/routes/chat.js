...
function canAccess(room, user) {
  if (!room.is_private) return true;
  if (user.role === 'admin' || room.created_by === user.id) return true;
  return !!db.prepare('SELECT 1 FROM room_members WHERE room_id=? AND user_id=?').get(room.id, user.id);
}

// 채팅방 목록 (공개 + 내가 접근 가능한 비밀방 상태 표시)
router.get('/rooms', requireAuth, (req, res) => {
  ...
  const rooms = rows.map((r) => ({
    ...
    joined: canAccess({ id: r.id, is_private: r.is_private, created_by: r.created_by }, req.user),
  }));
  res.json({ rooms });
});
...
router.post('/rooms', requireAuth, (req, res) => {
  ...
  const info = db.prepare('INSERT INTO chat_rooms (name, is_private, password, created_by) VALUES (?, ?, ?, ?)')
    .run(name.trim(), priv, hash, req.user.id);
  if (priv) db.prepare('INSERT OR IGNORE INTO room_members (room_id, user_id) VALUES (?, ?)').run(info.lastInsertRowid, req.user.id);
...
router.post('/rooms/:id/join', requireAuth, (req, res) => {
  ...
  if (!room.is_private) return res.json({ ok: true }); // 공개방은 그냥 통과
  if (canAccess(room, req.user)) return res.json({ ok: true });
...
router.get('/', requireAuth, (req, res) => {
  ...
  if (!room) return res.status(404).json({ error: '방을 찾을 수 없습니다.' });
  if (!canAccess(room, req.user)) return res.status(403).json({ error: '비밀방입니다. 입장코드가 필요해요.', locked: true });
...
router.post('/', requireAuth, (req, res) => {
  ...
  if (!canAccess(room, req.user)) return res.status(403).json({ error: '비밀방 입장코드가 필요해요.', locked: true });
  ...
  const info = db.prepare('INSERT INTO chats (author_id, content, room_id) VALUES (?, ?, ?)').run(req.user.id, content.trim(), roomId);
