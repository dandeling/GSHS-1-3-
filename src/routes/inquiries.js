...
router.get('/', requireAuth, (req, res) => {
  ...
  const rows = isAdmin
    ? db.prepare(`SELECT i.*, u.username FROM inquiries i JOIN users u ON u.id=i.user_id ORDER BY
        CASE i.status WHEN 'open' THEN 0 ELSE 1 END, i.id DESC`).all()
    : db.prepare('SELECT * FROM inquiries WHERE user_id=? ORDER BY id DESC').all(req.user.id);
  res.json({ inquiries: rows, isAdmin });
});

// 안읽은(미답변) 문의 개수 — 관리자 배지용
router.get('/pending-count', requireAdmin, (req, res) => {
  const c = db.prepare("SELECT COUNT(*) c FROM inquiries WHERE status='open'").get().c;
  res.json({ count: c });
});
...
router.post('/', requireAuth, (req, res) => {
  ...
  const info = db.prepare('INSERT INTO inquiries (user_id, title, content) VALUES (?, ?, ?)')
...
// 답변 (관리자) → 작성자에게 알림
router.post('/:id/answer', requireAdmin, (req, res) => {
  const inq = db.prepare('SELECT * FROM inquiries WHERE id=?').get(req.params.id);
  if (!inq) return res.status(404).json({ error: '문의를 찾을 수 없습니다.' });
  const { answer } = req.body || {};
  if (!answer?.trim()) return res.status(400).json({ error: '답변 내용을 입력하세요.' });
  db.prepare("UPDATE inquiries SET answer=?, status='answered', answered_at=datetime('now') WHERE id=?")
    .run(answer.trim(), inq.id);
  notify(inq.user_id, req.user.id, 'inquiry', null, `문의 "${inq.title.slice(0, 20)}"에 답변이 등록됐어요.`);
  res.json({ ok: true });
});
