...
function loadPoll(poll, userId) {
  ...
  `).all(poll.id);
  const mine = db.prepare('SELECT option_id FROM weekly_poll_votes WHERE poll_id=? AND user_id=?').get(poll.id, userId);
...
router.get('/', requireAuth, (req, res) => {
  const poll = db.prepare('SELECT * FROM weekly_polls WHERE active=1 ORDER BY id DESC LIMIT 1').get();
  res.json({ poll: loadPoll(poll, req.user.id), isAdmin: req.user.role === 'admin' });
});
...
router.post('/vote', requireAuth, (req, res) => {
  ...
  const opt = db.prepare('SELECT id FROM weekly_poll_options WHERE id=? AND poll_id=?').get(req.body?.option_id, poll.id);
  if (!opt) return res.status(400).json({ error: '선택지를 확인하세요.' });
  db.prepare(`INSERT INTO weekly_poll_votes (poll_id, user_id, option_id) VALUES (?, ?, ?)
    ON CONFLICT(poll_id, user_id) DO UPDATE SET option_id=excluded.option_id`).run(poll.id, req.user.id, req.body.option_id);
  res.json({ ok: true, poll: loadPoll(poll, req.user.id) });
});
...
router.post('/', requireAdmin, (req, res) => {
  ...
  db.prepare('UPDATE weekly_polls SET active=0 WHERE active=1').run();
  const info = db.prepare('INSERT INTO weekly_polls (question, created_by) VALUES (?, ?)').run(question.trim(), req.user.id);
  const ins = db.prepare('INSERT INTO weekly_poll_options (poll_id, idx, text) VALUES (?, ?, ?)');
  opts.forEach((t, i) => ins.run(info.lastInsertRowid, i, t));
